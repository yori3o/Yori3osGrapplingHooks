...
public class YoHooksEvents {
    ...
    private static final int SOUND_LENGTH_TICKS = 39; // ~2 секунды
    ...
    /*public static void ClientTick_CheckHookDiscard() {
        Player player = Minecraft.getInstance().player;
        if (player == null) return;
        ...
        HookEntity hook = hookData.getHook();
        if (hook != null) hookDiscardIfInvalid(player, hookData, hook);
    }



    
    public static void ClientTick_KeybindsHandler() {
        Player player = Minecraft.getInstance().player;
        if (player == null) return;
        ...
        HookEntity hook = hookData.getHook();
        if (hook != null) ClientTickKeybindsHandler(player, hookData, hook);
    }*/
    ...
    private static final void applyJumpImpulse(Player player, int agility_level) {
        ...
        final double FORWARD_VELOCITY = (0.47 + (agility_level * 0.097)) * PhysicVariables.jump_Multiplier; // Сила броска вперед
        final double UPWARD_VELOCITY = (0.41 + (agility_level * 0.043)) * PhysicVariables.jump_Multiplier;   // Сила броска вверх
        ...
        //double newX = target(currentMotion.x, targetX, lookVector.x);
        //double newZ = target(currentMotion.z, targetZ, lookVector.z);

        /*if (currentMotion.x >= 0) {
            newX = Math.max(currentMotion.x, targetX);
        } else {
            newX = Math.min(currentMotion.x, targetX);
        }*/
        ...
        double factorX = (lookVector.x + 1) / 2;

        double factorZ = (lookVector.z + 1) / 2;

        double newX = Math.abs(oldV.x) > Math.abs(targetX)
            ? (oldV.x + targetX) / 2
            : (oldV.z + targetZ) / 2;

        double newZ = Math.abs(oldV.z) > Math.abs(targetZ)
            ? oldV.z
            : targetZ;*/
